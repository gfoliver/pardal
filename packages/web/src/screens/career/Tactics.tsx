...
const roleLabel = (k: string) => cap(k);
...
export function Tactics() {
  ...
  const slot = selected != null ? v.slots[selected] : undefined;
  const roleOptions = slot ? allRoles().filter((r) => r.positions.includes(positionGroup(slot.position as Position))) : [];
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="secondary" onClick={autoPickLineup}>{t.autoPick}</Button>
      </div>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <Card>
          <CardContent className="p-3 sm:p-4">
            <div className="mx-auto max-w-md">
              <Pitch spots={spots} editable selectedId={selected} onSelect={(id) => setSelected(Number(id))} />
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex flex-col gap-4">
          ...
          {slot ? (
            <Card>
              <CardHeader action={<Button size="sm" variant="ghost" onClick={() => setPicking(true)}>{t.changePlayer}</Button>}>
                <CardTitle>{POS_SHORT[slot.position] ?? slot.position} · {slot.player?.name ?? "—"}</CardTitle>
              </CardHeader>
              <CardContent className="flex flex-col gap-3">
                {slot.player && (
                  <div className="flex items-center gap-2 text-sm">
                    <Overall value={slot.player.overall} />
                    <span className="text-fg-muted">{slot.player.name}</span>
                    {slot.player.injured && <Badge variant="gold">{t.out}</Badge>}
                  </div>
                )}
                <div className="flex flex-col gap-1.5">
                  <Label>{t.role}</Label>
                  <Select value={slot.role} onValueChange={(x) => slot.player && setPlayerRole(slot.player.playerId, x as RoleKey)} disabled={!slot.player}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{roleOptions.map((r) => <SelectItem key={r.key} value={r.key}>{roleLabel(r.key)}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
              </CardContent>
            </Card>
          ) : (
            <Card><CardContent className="py-4 text-sm text-fg-muted">{t.selectPlayerHint}</CardContent></Card>
          )}

          <Card>
            <CardHeader><CardTitle>{t.bench}</CardTitle></CardHeader>
            <CardContent className="flex max-h-60 flex-col gap-1 overflow-y-auto">
              {v.bench.map((p) => (
                <button
                  key={p.playerId}
                  disabled={selected == null}
                  onClick={() => selected != null && (setLineupSlot(selected, p.playerId), setSelected(null))}
                  className="flex items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm enabled:hover:bg-surface-2 disabled:opacity-70"
                >
                  <span className="w-8 text-2xs uppercase text-fg-faint">{POS_SHORT[p.position] ?? p.position}</span>
                  <span className={cn2(p.injured)}>{p.name}</span>
      ...
      <PlayerPicker open={picking} onClose={() => setPicking(false)} view={v} onPick={(id) => { if (selected != null) setLineupSlot(selected, id); setPicking(false); }} playerLabel={t.player} />
    </div>
  );
}

function cn2(injured: boolean): string {
  return injured ? "text-fg-faint line-through" : "text-fg";
}

function PlayerPicker({ open, onClose, view, onPick, playerLabel }: { open: boolean; onClose: () => void; view: TacticsView; onPick: (id: string) => void; playerLabel: string }) {
  const inXi = new Set(view.slots.map((s) => s.player?.playerId).filter(Boolean));
  const all = [...view.slots.map((s) => s.player).filter((p): p is NonNullable<typeof p> => Boolean(p)), ...view.bench];
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader><DialogTitle>{playerLabel}</DialogTitle></DialogHeader>
        <DialogBody className="flex max-h-[60vh] flex-col gap-1 overflow-y-auto">
          {all.map((p) => (
            <button key={p.playerId} onClick={() => onPick(p.playerId)} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm hover:bg-surface-2">
              <span className="w-8 text-2xs uppercase text-fg-faint">{POS_SHORT[p.position] ?? p.position}</span>
              <span className={p.injured ? "text-fg-faint line-through" : "text-fg"}>{p.name}</span>
              {inXi.has(p.playerId) && <Badge variant="muted">XI</Badge>}
              <span className="ml-auto tabular-nums text-fg-muted">{p.overall}</span>
            </button>
          ))}
        </DialogBody>
