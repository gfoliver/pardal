...
- Do not preserve backward compatibility. Remove obsolete paths instead of adding
  ...
  - Exception: persisted save files and the multiplayer protocol. A save on disk must keep
    loading, and a league in progress is pinned to its engine version.
...
- Grow the system in layers. Start from the smallest version that works end to end, and add
  each new capability on top of a product that already works. Never trade a working product
...
- Lean on the dependencies already in the project before writing your own implementation or
  adding packages. Do not assume a library lacks a capability without checking its
