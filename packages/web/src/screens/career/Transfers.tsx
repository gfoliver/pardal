...
const OFFER_LABEL: Record<OfferStatus, string> = { [OfferStatus.Pending]: "Pending", [OfferStatus.Accepted]: "Accepted", [OfferStatus.Rejected]: "Rejected", [OfferStatus.Completed]: "Signed", [OfferStatus.Withdrawn]: "Withdrawn" };
...
export function Transfers() {
  ...
  const submitOffer = () => {
    ...
    toast(ok ? `Offer lodged for ${offerFor.name}.` : `Couldn't lodge offer for ${offerFor.name}.`);
    setOfferFor(null);
  };
  ...
  const submitTerms = () => {
    ...
    toast(r.signed ? `${termsFor.playerName} signs!` : `${termsFor.playerName} wants higher wages.`);
  ...
  const targetCols: Column<TransferTarget>[] = [
    ...
    { key: "act", header: "", align: "right", cell: (r) => (
      <div className="flex justify-end gap-1">
        <Button size="sm" variant="secondary" onClick={() => openOffer(r)}>Offer</Button>
        <Button size="icon-sm" variant="ghost" aria-label="remove" onClick={() => removeTarget(r.playerId)}><X /></Button>
      </div>
    ) },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {signings.length > 0 && (
        <Card className="border-primary">
          <CardContent className="flex flex-col gap-2 py-4">
            <h2 className="text-xs font-bold uppercase tracking-wide text-primary">Personal terms</h2>
            {signings.map((s) => (
              <div key={s.playerId} className="flex items-center gap-3 text-sm">
                <span className="flex-1"><span className="font-medium text-fg">{s.playerName}</span> — fee agreed with {s.fromClubName} ({fmt.money(s.fee, { compact: true })})</span>
                <Button size="sm" variant="primary" onClick={() => openTerms(s)}>Agree terms</Button>
              </div>
            ))}
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="targets">
        <TabsList>
          <TabsTrigger value="targets">Targets{shortlist.length ? ` (${shortlist.length})` : ""}</TabsTrigger>
          <TabsTrigger value="mine">My offers{myOffers.filter((o) => o.status === OfferStatus.Pending).length ? ` (${myOffers.filter((o) => o.status === OfferStatus.Pending).length})` : ""}</TabsTrigger>
          <TabsTrigger value="received">Received{received.length ? ` (${received.length})` : ""}</TabsTrigger>
        </TabsList>

        <TabsContent value="targets">
          <Card><CardContent className="py-3">
            {shortlist.length === 0 ? (
              <p className="py-8 text-center text-sm text-fg-muted">Scout and add players to your shortlist.</p>
            ) : (
        ...
        <TabsContent value="mine">
          <Card><CardContent className="flex flex-col gap-2 py-4">
            {myOffers.length === 0 ? (
              <p className="py-6 text-center text-sm text-fg-muted">No offers made.</p>
            ) : (
              myOffers.map((o) => (
        ...
        <TabsContent value="received">
          <Card><CardContent className="flex flex-col gap-2 py-4">
            {received.length === 0 ? (
              <p className="py-6 text-center text-sm text-fg-muted">No offers received.</p>
            ) : (
              received.map((o) => (
                <div key={o.id} className="flex items-center gap-3 rounded-md border border-border px-3 py-2 text-sm">
      ...
      <Dialog open={offerFor !== null} onOpenChange={(o) => !o && setOfferFor(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Offer for {offerFor?.name}</DialogTitle></DialogHeader>
          <DialogBody className="flex flex-col gap-3">
            <div className="text-xs text-fg-muted">Value {offerFor && fmt.money(offerFor.value, { compact: true })} · {t.balance} {fmt.money(budget, { compact: true })}</div>
            <div className="flex flex-col gap-1.5"><Label>Fee</Label><MoneyInput value={fee} onValue={setFee} budget={budget} /></div>
          </DialogBody>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setOfferFor(null)}>Cancel</Button>
            <Button variant="primary" disabled={fee > budget} onClick={submitOffer}>Lodge offer</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={termsFor !== null} onOpenChange={(o) => !o && setTermsFor(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Terms · {termsFor?.playerName}</DialogTitle></DialogHeader>
          <DialogBody className="flex flex-col gap-3">
            <div className="text-xs text-fg-muted">Expected wage {termsFor && fmt.money(termsFor.expectedWage, { compact: true })}/wk</div>
            <div className="flex flex-col gap-1.5"><Label>Wage / week</Label><MoneyInput value={wage} onValue={setWage} step={5000} /></div>
            <div className="flex flex-col gap-1.5"><Label>Years</Label><NumberInput value={years} onValue={setYears} min={1} max={5} step={1} /></div>
          </DialogBody>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setTermsFor(null)}>Cancel</Button>
            <Button variant="primary" onClick={submitTerms}>Offer contract</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
