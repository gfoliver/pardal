...
import { MatchEventType, possessionPercent } from "@fut/engine";
import { getCatalog } from "@fut/i18n";
import { type SpatialPlayerView, type SpatialSnapshot } from "@fut/spatial";
...
export function Match() {
  ...
  const startQuick = () => {
    setQuick(simulateSpatial(HOME, AWAY, seed));
    setMode("quick");
  };
  const newMatch = () => {
    setSeed((s) => s + 1);
...
/* ---------------------------------------------------------------- Live view */
function LiveView({ live, locale, t }: { live: SpatialController; locale: "en" | "pt-BR"; t: ReturnType<typeof useApp>["t"] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-[1.3fr_1fr]">
        <Card><CardContent><SpatialPitch snap={snap} /></CardContent></Card>

        <Card>
          <CardHeader><CardTitle>{cat.phrase("timeline")}</CardTitle></CardHeader>
          <CardContent className="flex max-h-[520px] flex-col gap-0 overflow-y-auto p-0">
  ...
  return (
    <div className="relative aspect-[3/4] w-full overflow-hidden rounded-md border border-border-strong [background:repeating-linear-gradient(0deg,color-mix(in_srgb,var(--pitch-grass)_88%,#000)_0_8%,var(--pitch-grass)_8%_16%)]">
      ...
      {snap.players.map((p: SpatialPlayerView) => (
        <div
          key={p.id}
          className="absolute z-10 grid size-[22px] -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full text-[10px] font-bold tabular-nums text-[#04140e] [transition:left_90ms_linear,top_90ms_linear]"
          style={{
            left: `${toField(p.x)}%`,
            top: `${toField(p.y)}%`,
            background: p.teamId === HOME.id ? "var(--pos-mid)" : "var(--pos-att)",
            boxShadow: p.hasBall ? "0 0 0 2px #fff" : "0 1px 3px rgba(0,0,0,0.55)",
          }}
      ...
      <div
        className="absolute z-20 size-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#04140e] bg-white [transition:left_90ms_linear,top_90ms_linear]"
        style={{ left: `${toField(snap.ball.x)}%`, top: `${toField(snap.ball.y)}%`, boxShadow: "0 0 6px 1px rgba(255,255,255,0.75)" }}
      />
    </div>
  );
}
